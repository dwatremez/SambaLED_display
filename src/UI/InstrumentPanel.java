...
public class InstrumentPanel extends JPanel {
	...
	private void configureInstrumentExamples() {
		...
		instrumentItemPanel.setBackground(instrumentExamples.get(0)
				.getBackground());
		instrumentItemPanel.setBorder(BorderFactory.createMatteBorder(1, 0, 0,
				0, Color.LIGHT_GRAY));
	...
	public void addLine(InstrumentLine iLine, Point p) {
		if (!lines.isEmpty()) {
			...
			if (p.getY() > lines.get(lines.size() - 1).getY()
					&& p.getY() < instrumentLinesPanel.getHeight())
			...
			if (file.exists()) {
				try (BufferedReader reader = new BufferedReader(new FileReader(
					...
					while ((line = reader.readLine()) != null) {
						...
						int nbInstrument = Integer.parseUnsignedInt(String
								.valueOf(line.charAt(0)));
						...
						for (int i = 0; i < nbInstrument; i++) {
							...
							line = line.substring(line.indexOf(',') + 1);
							int pixel = Integer.parseInt(line.substring(0,
									line.indexOf(']')));
							// System.out.println(name + " " + type + " " +
							// shape + " " + pixel);
							InstrumentItem item = new InstrumentItem(name,
									type, shape, pixel);
	...
	public void save(File f) {
		...
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(
				f.getAbsolutePath()))) {
			for (int i = 0; i < lines.size(); i++) {
				writer.write(String.valueOf(lines.get(i).getInstruments()
						.size()));
				for (int j = 0; j < lines.get(i).getInstruments().size(); j++) {
					writer.write(lines.get(i).getInstruments().get(j)
							.toString());
				}
				writer.write("\n");
			}
