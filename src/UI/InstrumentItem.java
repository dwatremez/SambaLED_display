...
public class InstrumentItem extends DragDropFocusJPanel {
	...
	public String toString() {
		return "[" + this.name + "," + this.type + "," + this.shape + ","
				+ this.pixelNb + "]";
	}
	...
	public void setParameters(String name, String type, String shape,
			int pixelNb) {
		this.name = name;
		this.type = type;
		this.shape = shape;
	...
	public void paintComponent(Graphics g) {
		...
		g.setFont(nameFont);
		g.drawString(this.name, this.getWidth() / 2
				- g.getFontMetrics().stringWidth(this.name) / 2,
				nameFont.getSize());
		Font typeFont = new Font("Tahoma", Font.PLAIN, 9);
		g.setFont(typeFont);
		g.drawString(this.type, this.getWidth() / 2
				- g.getFontMetrics().stringWidth(this.type) / 2,
				nameFont.getSize() + typeFont.getSize());
		switch (shape) {
		case "Circle":
			g.fillOval(
					(int) (this.getWidth() / 2.0 - size / 4.0),
					(int) (this.getHeight() / 2.0 - size / 4.0)
							+ (nameFont.getSize() + typeFont.getSize()) / 2,
					(int) (size / 2.0), (int) (size / 2.0));
			break;
		case "Bar":
			g.fillRect(
					(int) (this.getWidth() / 2.0 - size / 3.0),
					(int) (this.getHeight() / 2.0 - size / 8.0)
							+ (nameFont.getSize() + typeFont.getSize()) / 2,
					(int) (2.0 / 3.0 * size), (int) (size / 4.0));
			break;
		case "Triple Bar":
			g.fillRect(
					(int) ((this.getWidth() / 2.0 - size / 8.0) - size / 3.0),
					(int) (this.getHeight() / 2.0 - size / 4.0)
							+ (nameFont.getSize() + typeFont.getSize()) / 2,
					(int) (size / 4.0), (int) (size / 2.0));
			g.fillRect(
					(int) ((this.getWidth() / 2.0 - size / 8.0)),
					(int) (this.getHeight() / 2.0 - size / 4.0)
							+ (nameFont.getSize() + typeFont.getSize()) / 2,
					(int) (size / 4.0), (int) (size / 2.0));
			g.fillRect(
