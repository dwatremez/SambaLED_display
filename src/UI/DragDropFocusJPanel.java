...
public class DragDropFocusJPanel extends JPanel implements Cloneable {
	...
	private Component findComponentUnderMouse() {
		...
		return SwingUtilities.getDeepestComponentAt(container, location.x,
				location.y);
	}
	...
	public void deleteItemAndEmptyLine(InstrumentItem source) {
		...
		if (source.getParent().getParent() != null)
			if (source.getParent().getParent().getClass() == InstrumentLine.class) {
				...
				// System.out.println("suppr: " + source + " from " +
				// sourceLine.toString());
				// System.out.println("origin: " +
				// sourceLine.getInstruments().toString());
			...
			if (source.getClass() == InstrumentItem.class
				...
				if ((lineSelected = findInstrumentLine(findComponentUnderMouse())) != null) {
					...
					if (!lineSelected.getInstruments().isEmpty()) {
						...
						for (int i = 0; i < lineSelected.getInstruments()
								.size(); i++) {
							if (p.getX() < lineSelected.getInstruments().get(i)
									.getX()
									+ lineSelected.getLabel().getWidth()) {
								position = i;
								inLine = true;
								break;
							}
						...
						if (position != 0) {
							if (p.getX() < lineSelected.getInstruments()
									.get(position - 1).getX()
									+ lineSelected.getInstruments()
											.get(position - 1).getWidth()) {
								myGlass.setBadFocus();
								inLine = false;
							}
						}
						...
						if (inLine) {
							...
							if (position == lineSelected.getInstruments()
									.size())
								p1.setLocation(lineSelected.getWidth(),
										lineSelected.getHeight());
							else
								p1.setLocation(
										lineSelected.getLabel().getWidth()
												+ lineSelected.getInstruments()
							...
							p0.setLocation(
									p0.getX()
											+ lineSelected.getLabel()
													.getWidth(),
									p0.getY()
											- findInstrumentPanel(
													findComponentUnderMouse())
													.getScroll());
							p1.setLocation(p1.getX() - p0.getX(), p1.getY());

							if (p0.getY() + p1.getY() > findInstrumentPanel(
									findComponentUnderMouse()).getScrollPanel()
									.getHeight())
								p1.setLocation(
										p1.getX(),
										findInstrumentPanel(
												findComponentUnderMouse())
												.getScrollPanel().getHeight()
												- p0.getY());

							Point[] focusP = { p0, p1 };
							myGlass.setFocus(focusP,
									analogColor(lineSelected.getBackColor(), 2));
						}
					}
				...
				// Draw focus on Panel
					...
					if (!panelSelected.getLines().isEmpty()) {
						...
						if (p.getY() < panelSelected.getScrollPanel()
								.getHeight()) {
							position = panelSelected.getLines().size();
							inPanel = true;
						}
						for (int i = 0; i < panelSelected.getLines().size(); i++) {
							if (p.getY() < panelSelected.getLines().get(i)
									.getY()
									- panelSelected.getScroll()) {
								position = i;
								break;
							}
						}
						...
						if (inPanel) {
							...
							if (position == panelSelected.getLines().size())
								p1.setLocation(panelSelected.getScrollPanel()
										.getWidth(),
										panelSelected.getScrollPanel()
												.getHeight() - p0.getY());
							else
								p1.setLocation(panelSelected.getScrollPanel()
										.getWidth(), panelSelected.getLines()
										.get(position).getY()
										- p0.getY());

							p0.setLocation(p0.getX(),
									p0.getY() - panelSelected.getScroll());

							if (p0.getY() + p1.getY() > panelSelected
									.getScrollPanel().getHeight())
							...
							Point[] focusP = { p0, p1 };
							myGlass.setFocus(focusP,
									analogColor(panelSelected.getBackColor()));
						}
					} else {
						if (p.getY() < panelSelected.getScrollPanel()
								.getHeight()) {
							...
							p0.setLocation(0, 0);
							p1.setLocation(panelSelected.getScrollPanel()
									.getWidth(), panelSelected.getScrollPanel()
									.getHeight());
		...
		private Color analogColor(Color c, int i) {
			...
			if (hsb[1] > 0.1)
				return new Color(Color.HSBtoRGB(
						(float) (hsb[0] + i * 30.0 / 360.0), hsb[1], hsb[2]));
			else if (hsb[2] < 0.4)
				return c.brighter();
			else
				return c.darker();
		...
		public void mouseReleased(MouseEvent event) {
			...
			if (source.getClass() == InstrumentItem.class) {
				...
				// Move and Copy InstrumentItem in new Line in InstrumentPanel
					...
					lineSelected.addInstrument(clone);
					panelSelected.addLine(
							lineSelected,
							getPointInComponent(event,
									panelSelected.getInstrumentLinesPanel()));
